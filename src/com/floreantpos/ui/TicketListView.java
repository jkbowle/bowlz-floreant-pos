	...
	private class TicketListTableModel extends ListTableModel {
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			switch(columnIndex) {
				...
				case 1:
					return getTicketNumberName(ticket);
        ...
        private Object getTicketNumberName(Ticket ticket) {
            Integer tableNumber = Integer.valueOf(ticket.getTableNumber());
            String name = ticket.getCustName();
            String ret = tableNumber.toString();
            if(name != null && !"".equalsIgnoreCase(name.trim()))
            {
               if(Ticket.TAKE_OUT == tableNumber)
               {
                   ret = "TAKE OUT (" +name+")";
               }
               else
