		...
		try {
			...
			if (ticketsToSettle.size() > 1 && tenderedAmount < dueAmountBeforePaid) {
				...
				return;
                        ...
                        TenderAndChange tndChg = null;
			if(ticketsToSettle.size()<2)
                        {
                            Double amountGiven = new Double(tenderedAmount); 
                            tndChg = new TenderAndChange(amountGiven,ticketsToSettle.get(0).getDueAmount());
                        }
			...
			try {
				for (Ticket ticket : ticketsToSettle) {
					PosPrintService.printTicket(ticket,true,tndChg);
				}
			}catch(Exception ee) {
