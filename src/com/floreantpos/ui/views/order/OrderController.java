...
public class OrderController implements OrderListener, CategorySelectionListener, GroupSelectionListener, ItemSelectionListener, ModifierSelectionListener {
	...
	public void payOrderSelected(Ticket ticket) {
        ...
        private static void updateOrder(Ticket ticket) {
		if (ticket == null)
			return;
        ...
        public static Ticket refresh(Ticket ticket)
        {
            return TicketDAO.getInstance().refresh(ticket);
        }
        
	public static void saveOrder(Ticket ticket) {
               System.out.println("Ticket: "+ticket.getId()+ " Modified: "+(ticket.getModifiedTime()==null?0L:ticket.getModifiedTime().getTime()));
               for(TicketItem item: ticket.getTicketItems())
               {
                  System.out.println("TicketItem: "+item.getId()+" Modified: "+(item.getModifiedTime()==null?0L:item.getModifiedTime().getTime())); 
               }    
		if (ticket == null)
			return;
		...
		if(ticket.getId() == null)
                {
                    ticket.setId(TicketDAO.getInstance().save(ticket));
