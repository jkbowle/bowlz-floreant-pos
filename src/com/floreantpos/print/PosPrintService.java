...
import com.floreantpos.ui.util.TenderAndChange;
...
public class PosPrintService {
	...
	static void printFourthColumn(PosPrinter printer, String text, int fourthColumnLength) {
        ...
        public static void printTicket(final Ticket ticket) throws Exception {printTicket(ticket,false,null);}
        
	public static void printTicket(final Ticket ticket, final boolean popDrawer, final TenderAndChange tndChg) throws Exception {
		Job job = new Job() {
			...
			public Object run() {
				...
				try {
					...
					posPrinter.beginLine(PosPrinter.SIZE_0);
					posPrinter.printText(StringUtils.leftPad("TICKET TOTAL " + ":", 32));
					...
					posPrinter.endLine();
                                        ...
                                        if(ticket.getGratuity()!= null && ticket.getGratuity().getAmount() != null 
                                                && ticket.getGratuity().getAmount()>0)
                                        {
                                            posPrinter.beginLine(PosPrinter.SIZE_0);
					...
					if(tndChg != null)
                                        {
                                            posPrinter.beginLine(PosPrinter.SIZE_0);
                                            posPrinter.printText(StringUtils.leftPad("TOTAL TENDERED " + ":", 32));
                                            posPrinter.printText(StringUtils.leftPad(Application.formatNumber(tndChg.getGivenAmt()), 10));
                                            posPrinter.endLine(); 
                                            posPrinter.beginLine(PosPrinter.SIZE_0);
                                            posPrinter.printText(StringUtils.leftPad("CHANGE " + ":", 32));
                                            posPrinter.printText(StringUtils.leftPad(Application.formatNumber(tndChg.getChange()),10));
                                            posPrinter.endLine();
                                        }
		...
		Job job = new Job() {
			...
			public Object run() {
				...
				try {
					...
					posPrinter = new PosPrinter(PrintConfig.getJavaPosKitchenPrinterName(), null);
					...
					posPrinter.endLine();
                                        // Added table to number to kitchen receipt 12/28/2013
                                        posPrinter.beginLine(PosPrinter.SIZE_0);
                                        posPrinter.printText(com.floreantpos.POSConstants.TABLE_NO);
                                        if(ticket.isTakeOut())
                                        {
    ...
    private static void testReceiptPrinter() 
    {
        PosPrinter posPrinter = new PosPrinter(PrintConfig.getJavaPosReceiptPrinterName(), PrintConfig.getCashDrawerName());
        posPrinter.printImage(null);
        posPrinter.beginLine(0);
        posPrinter.printText("========  ======= =======");
        posPrinter.printText("========  TESTING RECEIPT =======");
        posPrinter.printText("========  TESTING =======");
        posPrinter.printText("========  TESTING RECEIPT =======");
        posPrinter.printText("========  ======= =======");
        posPrinter.endLine();
        posPrinter.printCutPartial();
    ...
    private static void testKitchenPrinter() 
    {
        PosPrinter posPrinter = new PosPrinter(PrintConfig.getJavaPosKitchenPrinterName(), null);
        posPrinter.printImage(null);
        posPrinter.beginLine(0);
        posPrinter.printText("========  ======= =======");
        posPrinter.printText("========  TESTING KITCHEN =======");
        posPrinter.printText("========  TESTING =======");
        posPrinter.printText("========  TESTING KITCHEN =======");
        posPrinter.printText("========  ======= =======");
        posPrinter.endLine();
